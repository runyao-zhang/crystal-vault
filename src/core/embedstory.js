...
import { patchBody, splitCard, stripBodyPrefix } from "./frontmatter.js";
...
export function createEmbedStory(ctx, opts = {}) {
  ...
  function syncBar() {
    ...
    const n = marqueeSel(fake).length;
  ...
  async function writeStoryLink(fromPath, toPath) {
    ...
    undoState = { path: card.path, prev: base, base: res.content };
  ...
  async function saveReason() {
    ...
    // base 换成最新那份。
    if (undoState && undoState.path === card.path) undoState.base = res.content;
    closeReason();
    say("理由记下了。", true);
    render(view.path, { keepCamera: true });
  }
  ...
  async function undoWrite() {
    ...
    let res;
    try {
      res = await ctx.adapter.writeCard(u.path, u.prev, { base: u.base });
    } catch (e) {
      say("撤销失败：" + ((e && e.message) || e), false);
      return;
    }
    if (!res || !res.ok) {
      say(res && res.reason === "conflict" ? "这张卡在别处被改过，没有撤销。" : "撤销失败。", false);
      return;
    }
    const card = ctx.model.byPath.get(u.path);
